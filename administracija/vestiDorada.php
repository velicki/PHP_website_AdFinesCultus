            ...
            <div id="vesti">
                ...
                <div id="svesti">
                    ...
                    if (isset($_POST['update']))
                    {
                        ...
                        $novitext = $prepravka;                       
                        $id      = $_POST['id'];
                        $sql     ="UPDATE vesti SET naslov='$novinaslov' , autor='$noviautor' , kratakt='$novinaslovni_tekst' , tekst='$novitext'  WHERE id='$id'";
                 
                        $res     =  mysqli_query($con, $sql) or die ("Culd not update.".  mysqli_error());
                        mysqli_close($con);
                        echo "<meta http-equiv='refresh' content='0;url=vestiDorada.php?more=$id'>";
                    }
                    if (isset($_POST['upload']))
                    {
                        ...
                        else 
                        {
                            ...
                            else 
                            {
                                    ...
                                    $res = mysqli_query($con, "SELECT * FROM vesti");
                                ...
                                $location1 = '../Images/vesti/';
                                    if (move_uploaded_file($file, $location1.$image_name))
                                    {
                                        
                                        $id      = $_POST['id'];
                                         $file2 = mysqli_query($con, "SELECT * FROM vesti WHERE id='$id'");
                                         while ($row2 = mysqli_fetch_array($file2))
                                         {
                                            $location2 = '../Images/vesti/';
                                            $location3 = $row2[4];
                                            $filename1 = "$location2$location3";
                                            unlink($filename1);
                                         }  
                                        $sql     ="UPDATE vesti SET imeslike='$image_name' WHERE id='$id'";
                                        $res     =  mysqli_query($con, $sql) or die ("Nije uspešno updejtovana slika.".  mysqli_error());
                                       
                                           
                                        mysqli_close($con);
                                        echo "<meta http-equiv='refresh' content='0;url=vesti.php'>";
                                    }
                                    else
                                        {
                    ...
                    ?>
                            ...
                            <form action="vestiDorada.php" name="myform" id="myform" method="POST" enctype="multipart/form-data">
                    
                            
                                <div class="divimgonama">
                                    <div><img  src="../Images/vesti/<?php echo $row[4];?>"  class="novostislikaadmin"  /></div>
                                    <div class="vestDoradaSlika">
                                        Izaberi novu sliku:
                                        <input type="file" name="image"><br> 
                                        Preporučena rezolucija slike je 530px/278px
                                ...
                                <div class="divimgonama">
                                <input type="submit" name="update" value="Dodaj Promene u Vesti" onClick="javascript:submit_form();">
                                </div>
